...
const MoviesPage = () => {
  ...
  const getOccupancyStatus = (sessionCount: number, maxSessions: number) => {
    const occupancy = (sessionCount / maxSessions) * 100;
    if (occupancy >= 80) return { text: "Çok Popüler", color: "text-red-400" };
    if (occupancy >= 60) return { text: "Popüler", color: "text-orange-400" };
                ...
                {isLoggedIn && user ? (
                  <div className="relative">
                    <button
                      ...
                      className="flex items-center space-x-3 bg-white/10 backdrop-blur-md rounded-full px-4 py-2 border border-white/20 hover:bg-white/20 transition-colors">
                      <img
                        src={user.avatar}
                        alt="Profile"
                    ...
                    {showUserMenu && (
                      <motion.div
                        ...
                        className="absolute right-0 mt-2 w-64 bg-black/90 backdrop-blur-md rounded-2xl border border-white/20 shadow-xl z-50">
                  ...
                  <Button
                    onClick={() => {
                      // Demo için sahte kullanıcı girişi
                      const demoUser = {
                        id: "1",
                        firstName: "Eren",
                        lastName: "Coşkun",
                        email: "eren@cinefyai.com",
                        avatar:
          ...
          <div className="max-w-7xl mx-auto">
            <motion.div
              ...
              <p className="text-xl text-gray-300 max-w-3xl mx-auto">
                En yeni ve popüler filmleri keşfedin, biletinizi hemen satın
                alın
              </p>
            </motion.div>
                      ...
                      <div className="space-y-2">
                        {movie.cinemas.map((cinema) => {
                          ...
                          return (
                            <div
                              key={cinema.id}
                              className="p-3 bg-white/5 rounded-lg hover:bg-white/10 transition-colors gap-3">
                              {/* Üst kısım - Sinema bilgileri */}
                              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-3">
                                <div className="flex flex-col sm:flex-row sm:items-center space-y-1 sm:space-y-0 sm:space-x-3 flex-1">
                                ...
                                <div className="flex items-center gap-3">
                                  <span
                                    className={`text-xs inline-block px-2 py-1 rounded-full bg-black/20 ${occupancyStatus.color} whitespace-nowrap`}>
                                    {occupancyStatus.text}
                                  </span>
                                  <div className="text-right">
                                    <div className="text-emerald-400 font-medium text-base">
                                      {formatPrice(cinema.priceRange.min)}
                                      {cinema.priceRange.max > 0 && (
                                        <span className="text-yellow-400 ml-1">
                                          - {formatPrice(cinema.priceRange.max)}
                                        </span>
                                      )}
                                    </div>
                                    <div className="text-gray-400 text-xs">
                                      En yakın: {cinema.nextSession}
                                    </div>
                                    <div className="text-gray-400 text-xs">
                                      {cinema.sessionCount} seans
                                    </div>
                                  </div>
                                </div>
                              </div>

                              {/* Alt kısım - Seansları Gör butonu */}
                              <div className="pt-2 border-t border-white/10">
                                <Link
                                  href={`/cinema/${cinema.id}/movie/${movie.id}`}
                                  className="block">
                                  <Button
                                    size="sm"
