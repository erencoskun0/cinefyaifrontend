...
const SessionsPage = () => {
  ...
  const [halls] = useState<Hall[]>([
    { id: "1", name: "Salon 1 - IMAX", capacity: 250 },
    { id: "2", name: "Salon 2 - VIP", capacity: 120 },
    { id: "3", name: "Salon 3 - Standart", capacity: 180 },
  ]);

  const [sampleAnalytics] = useState<SessionAnalytics>({
    sessionId: "1",
    totalTickets: 250,
    soldTickets: 70,
    revenue: 3150,
  ...
  const filteredSessions = sessions.filter((session) => {
    const matchesSearch =
      session.movie?.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      session.hall?.name.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const handleViewAnalytics = (session: Session) => {
    setSelectedSessionAnalytics(sampleAnalytics);
    setShowAnalyticsModal(true);
  };

  const handleInputChange = (field: keyof SessionForm, value: any) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value,
    }));
  ...
  const getOccupancyColor = (occupancyRate: number) => {
    if (occupancyRate >= 80) return "text-red-400";
    if (occupancyRate >= 60) return "text-orange-400";
    if (occupancyRate >= 40) return "text-yellow-400";
    return "text-green-400";
  };

  const getOccupancyBg = (occupancyRate: number) => {
    if (occupancyRate >= 80) return "bg-red-500/20";
    if (occupancyRate >= 60) return "bg-orange-500/20";
    if (occupancyRate >= 40) return "bg-yellow-500/20";
          ...
          <div className="relative flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-purple-200 mt-2 text-lg">
                  Film seanslarınızı yönetin ve rezervasyonları takip edin
                </p>
              </div>
            </div>
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Button
                ...
                className="bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-medium px-6 py-3 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300">
                <Plus className="h-5 w-5 mr-2" />✨ Seans Ekle
              </Button>
            </motion.div>
          </div>
        </div>
        ...
        <motion.div
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <Input
                placeholder="Film adı veya salon ara..."
                value={searchTerm}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredSessions.map((session, index) => {
            const occupancyRate = Math.round(
              ((session.totalSeats - session.availableSeats) /
                session.totalSeats) *
                100
            );
                      ...
                      <div className="flex-1">
                        ...
                        <div className="space-y-1 text-sm text-gray-300">
                          ...
                          <div className="flex items-center">
                            ...
                            {formatDuration(session.movie?.duration || 0)})
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white/5 rounded-lg p-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center">
                          ...
                          <span className="text-sm text-gray-300">Fiyat</span>
                        </div>
                    ...
                    <div className="bg-white/5 rounded-lg p-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center">
                          ...
                          <span className="text-sm text-gray-300">Doluluk</span>
                        </div>
                        <div
          ...
          <div className="space-y-4 p-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => handleInputChange("hallId", e.target.value)}
                className="w-full px-3 py-2 bg-white/5 border border-white/20 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500">
                <option value="">Salon seçin</option>
                {halls.map((hall) => (
                  <option key={hall.id} value={hall.id} className="bg-gray-800">
                    {hall.name} ({hall.capacity} kişi)
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Input
                ...
                onChange={(e) =>
                  handleInputChange("vipPrice", parseFloat(e.target.value) || 0)
                }
                className="bg-white/5 border-white/20 text-white"
              />
            </div>
            ...
            <div className="flex justify-end space-x-4 pt-4">
              ...
              <Button onClick={handleSaveSession}>
                ...
                {editingSession ? "Güncelle" : "Kaydet"}
              </Button>
            </div>
          </div>
        </Modal>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-blue-500/20 rounded-lg p-4 text-center">
                  <div className="text-2xl font-bold text-blue-400">
                    {selectedSessionAnalytics.soldTickets}
                  </div>
                  <div className="text-sm text-blue-300">Satılan Bilet</div>
                </div>
                <div className="bg-green-500/20 rounded-lg p-4 text-center">
                  <div className="text-2xl font-bold text-green-400">
                    ₺{selectedSessionAnalytics.revenue}
                  </div>
                  <div className="text-sm text-green-300">Gelir</div>
                </div>
                <div className="bg-purple-500/20 rounded-lg p-4 text-center">
                  <div className="text-2xl font-bold text-purple-400">
                    %{selectedSessionAnalytics.occupancyRate}
                  </div>
                  <div className="text-sm text-purple-300">Doluluk</div>
                </div>
                <div className="bg-orange-500/20 rounded-lg p-4 text-center">
                  <div className="text-2xl font-bold text-orange-400">
                    {selectedSessionAnalytics.totalTickets -
                      selectedSessionAnalytics.soldTickets}
                  </div>
                  <div className="text-sm text-orange-300">Boş Koltuk</div>
                </div>
              </div>

              {/* Seat Map Preview */}
              <div className="bg-white/5 rounded-lg p-6">
                <h3 className="text-white font-medium mb-4">Koltuk Durumu</h3>
                <div className="text-center mb-4">
                  <div className="w-full h-8 bg-gradient-to-r from-purple-600 to-blue-600 rounded-lg flex items-center justify-center">
                    <span className="text-white font-medium">PERDE</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-12 gap-1 max-h-64 overflow-auto">
                  {Array.from({ length: 60 }, (_, i) => {
                    ...
                    const number = (i % 12) + 1;
                    const isOccupied = Math.random() > 0.7; // Sample data
                ...
                <div className="flex items-center justify-center space-x-6 mt-4 text-sm">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-gray-300">Dolu</span>
                  </div>
                </div>
              </div>
            </div>
